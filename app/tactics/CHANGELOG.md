...
- Locked offense custom-route direction: when `Players: Locked` is enabled, selecting an offensive player and using `Ctrl+click` (`Cmd+click` on Mac) now creates a custom arrow from that player toward the mouse direction for manual route sketching; these custom arrows are also persisted in saved plays.
